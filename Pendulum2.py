...
class Pendulum(Particle):
    ...
    def update_RK(self, deltaT):
        ...
        vel = np.array([0.0, 0.0, self.ang_velocity[2] + (k_b[0]*0.5) ] , dtype = float)
        acc = self.damped_sho(ang, vel)
        k_b[1] = acc *deltaT

        k_a[2] = (self.ang_velocity[2] + (k_b[1]/2.)) * deltaT
        ang = self.angle + (k_a[1]*0.5)
        vel = np.array([0.0, 0.0, self.ang_velocity[2] + (k_b[1]*0.5)] , dtype = float)
        acc = self.damped_sho(ang, vel)
        k_b[2] = acc *deltaT

        k_a[3] = (self.ang_velocity[2] + k_b[2]) * deltaT
        ang = self.angle + (k_a[2])
        vel = np.array([0.0, 0.0, self.ang_velocity[2] + (k_b[2])] , dtype = float)
        acc = self.damped_sho(ang, vel)
        k_b[3] = acc *deltaT
    ...
    def double_pendulum_acc_update(self):
...
#        den = 2*m_1 + m_2 - m_2 *np.cos(2*angle_1 + 2* angle_2)
#
#
 #       num1 = (-9.81)*(2*m_1 + m_2) * np.sin(angle_1)
  #      num2 = -m_2 * (9.81) * np.sin(angle_1 + angle_2)
   #     num3 = 2*sin(angle_1 + angle_2) * m_2 
    #    num4 = (ang_vel_2)**2 * l_2 + (ang_vel_1)**2 *l_2 * np.cos(angle_1 - angle_2)
#
 #       num = num1 + num2 + (num3 * num4)
#
 #       acc_1 = num / (l_1 * den)
#
 #       num1 = 2*sin(angle_1 - angle_2)
  #      num2 = (ang_vel_1)**2 * l_1 * (m_1 + m_2)
   #     num3 = (9.81)*(m_1 + m_2) * np.cos(angle_1)
    #    num4 = (ang_vel_2)**2 * l_2 * m_2 * np.cos(angle_1 - angle_2)
#
 #       num = num1 * (num2 + num3 + num4)
#
 #       acc_2 = num / (l_2 *den)