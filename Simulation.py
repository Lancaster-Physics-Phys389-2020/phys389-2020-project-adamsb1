...
pendulum2 = np.array([np.array([-8.,-6.,0]),np.array([0.,0,0]),np.array([0,0,0]),'ball', 1, 0., 1., [0.,0.,0.], [0.,0.,0.], [-4.,-3.,0.], 0.0, 0.0, 0.0, df])
...
if NumberOfPendulums == 1:
    ...
    time_analysis = 'N'
    pendulums.run_simulation(endT, deltaT, update_method, time_analysis)   #Calls the method which runs the simulation.
    ...
    for i in range(0,len(initial_x_values)):
        for j in range(0,len(initial_y_values)):        #Loops through list of initial positions stated earlier.
            ...
            time_analysis = 'Y'
            time_to_flip = pendulums.run_simulation(endT, deltaT, update_method, time_analysis) #Runs time analysis for double pendulum
    ...
    df.to_pickle('Pendulum_time analysis_06_04.csv')    #Uses Pandas to save all time analysis data to a .csv file.
