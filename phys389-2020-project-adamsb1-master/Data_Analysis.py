...
data1 = pd.read_pickle(r'Euler_update_r.csv')
data2 = pd.read_pickle(r'Euler_update_e.csv')
data3 = pd.read_pickle(r'Euler_update_c.csv')

plt.plot([data1['position'][i][0] for i in range(len(data['position']))], [data1['position'][i][1] for i in range(len(data['position']))], 'red', label = 'position r')
plt.plot([data2['position'][i][0] for i in range(len(data['position']))], [data2['position'][i][1] for i in range(len(data['position']))], 'blue', label = 'position e')
plt.plot([data3['position'][i][0] for i in range(len(data['position']))], [data3['position'][i][1] for i in range(len(data['position']))], 'green', label = 'position c')
